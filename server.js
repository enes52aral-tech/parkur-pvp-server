...
const registeredPlayers = new Map(); // name → { lastSeen, id }
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'lobby_join': {
        ...
        // Bekleyen hediyeler varsa gönder
        const pending = giftQueue.get(ws.playerName)||[];
        if(pending.length){ setTimeout(()=>{ pending.forEach(g=>send(ws,{type:'gift_received',gift:g})); giftQueue.delete(ws.playerName); },1000); }
      ...
      case 'gift': {
        // Hediyeyi kuyruğa ekle, alıcı online ise anında gönder
...
// Hediye bekleyen kuyruk: name → [gift, gift, ...]
const giftQueue = new Map();
